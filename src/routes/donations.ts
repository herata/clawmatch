...
import { createDonation, createDonationEvent, getDonationById, updateDonationStatus } from '../repos/donations'
...
export const registerDonationRoutes = (app: Elysia, env: RuntimeEnv): void => {
  app.post('/v1/donations/x402/checkout', async ({ request, body, status }) => {
    ...
    const paymentResponseHeader = request.headers.get('payment-response') || request.headers.get('x-payment-response')

    const result = await runIdempotent(env.DB, `x402_checkout:${idempotencyKey}`, idempotencyKey, async () => {
  ...
  app.post('/v1/donations/x402/webhook', async ({ request, status }) => {
    if (!env.X402_WEBHOOK_SECRET) {
      return status(500, { ok: false, error: 'x402_webhook_secret_not_configured' })
    }

    const signature = request.headers.get('x-x402-signature') || request.headers.get('payment-signature')
    if (!signature) {
    ...
    const eventStatus = asString(payload.status) as 'pending' | 'confirmed' | 'failed' | null

    if (!providerEventId) return status(400, { ok: false, error: 'provider_event_id_required' })
    if (!donationId) return status(400, { ok: false, error: 'donation_id_required' })
    if (!eventStatus || !['pending', 'confirmed', 'failed'].includes(eventStatus)) {
